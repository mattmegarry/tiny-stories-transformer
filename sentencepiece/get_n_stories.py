...
def write_n_stories(num_stories):
    count = 0
    training_filename = './sentencepiece/TinyStories-' + str(num_stories) + '.txt'

    def transmute_stories(num_stories=num_stories, count=count):
        if os.path.exists(training_filename):
            os.remove(training_filename)
        with open('./sentencepiece/TinyStories-train.txt') as source, open(training_filename, 'a') as train_data:
            line_accumulator = ""
            for line in source:
